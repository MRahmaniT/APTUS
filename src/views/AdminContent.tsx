...
export default function AdminContent() {
  ...
  const addMediaUrl = () => {
    setDraft((current) => ({
      ...current,
      media: [...current.media, { mediaType: "image", url: "", caption: "", alt: "", sortOrder: current.media.length }],
    }));
  };
  ...
  const removeMedia = (index: number) => {
    setDraft((current) => ({ ...current, media: current.media.filter((_, entryIndex) => entryIndex !== index) }));
  };
